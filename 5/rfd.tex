...
\begin{align}
    ...
    j&=0
\end{align}
 substituting $j =0$ gives the passband
range for our bandpass filter as $4$ kHz - $5.2$ kHz.  Hence, the un-normalized discrete time filter
passband frequencies are $F_{p1} = 4$ kHz
and $F_{p2} = 5.2$ kHz. \\
The corresponding normalized digital filter passband frequencies are
\begin{align}
    \omega_{p1} &= 2\pi\frac{F_{p1}}{F_s} = 0.17 \pi\\
    \omega_{p2} &= 2\pi\frac{F_{p2}}{F_s}  =0.22 \pi
\end{align}
...
\begin{align}
    F_{s1} &= 4-0.3 = 3.7 \text{KHz}\\
    F	_{s2} &= 5.2+0.3 = 5.5  \text{KHz}
\end{align}
\begin{align}
    \omega_{s1} &= 2\pi\frac{F_{s1}}{F_s} = 0.154 \pi\\
...
$\Omega_{p1} = 0.2736$, $\Omega_{p2} = 0.3600$ and $\Omega_{s1} = 0.2467$, $\Omega_{s2} = 0.3760$
respectively.
...
\end{equation}
where $\Omega_0 = \sqrt{\Omega_{p1}\Omega_{p2}} = 0.3138$ and $B = \Omega_{p2} - \Omega_{p1} = 0.0864$.

Substituting $\Omega_{s1}$ and $\Omega_{s2}$ in \eqref{eq:freq_transform} we obtain the stopband edges of lowpass filter 
\begin{align}
    \Omega_{Ls1} &= \frac{\Omega_{s1}^2 - \Omega_0^2}{B\Omega_{s1}} = -1.770\\
    \Omega_{Ls2} &= \frac{\Omega_{s2}^2 - \Omega_0^2}{B\Omega_{s2}} = 1.317
\end{align}
And we choose the minimum of these two stopband edges
\begin{align}
    \Omega_{Ls} = \mbox{min}(\vert \Omega_{Ls_1}\vert,\vert \Omega_{Ls_2}\vert) = 1.317.
\end{align}
...
we get $N\geq 4$ and $0.505 \leq \epsilon \leq 0.61$\\
The below code plots \eqref{eq:mag_freq_response} for different values of $\epsilon$ .
\begin{lstlisting}
https://github.com/Raghava11020/Signals-and-Systems/blob/main/5/codes/p1.py
\end{lstlisting}
\begin{figure}[htbp]
    \centering
    \includegraphics[width=1\linewidth]{fig/p1.png}
    \caption{The Analog Low-Pass Frequency Response for $0.505 \leq \epsilon \leq 0.61$}
...
\begin{lstlisting}
https://github.com/Raghava11020/Signals-and-Systems/blob/main/5/codes/skgen.c
\end{lstlisting}
The poles obtained are formulated in the table below.
\input{tables/table1.tex}
The below code plots the pole-zero plot.
\begin{lstlisting}
https://github.com/Raghava11020/Signals-and-Systems/blob/main/5/codes/p2.py
\end{lstlisting}
\newpage
\begin{figure}[htbp]
    \centering
    \includegraphics[width=1\linewidth]{fig/p2.png}
...
\end{align}
Substituting respective values in \eqref{eq:Gain_eq_LP} we get $G_{LP}=0.2474$
\begin{align}
     H_{a,LP}(s_L) &= \frac{0.2474}{\brak{s_L-s_1}\brak{s_L-s_2}\brak{s_L-s_3}\brak{s_L-s_4}}\\
    &= \frac{0.2474}{s_{L}^4 + 0.9574s_{L}^3 +1.45846s_{L}^2 + 0.7475303s_{L} +0.27732667}\label{eq:design}
\end{align}
\begin{lstlisting}
https://github.com/Raghava11020/Signals-and-Systems/blob/main/5/codes/p3.py
\end{lstlisting}
\newpage
\begin{figure}[htbp]
    \centering
    \includegraphics[width=1\linewidth]{fig/p3.png}
...
\begin{align}
     H_{a,BP}\brak{s} &= \frac{6.39\times 10^{-5}s^4}{s^8 + 0.0828s^7 + 0.4052s^6 + 0.025s^5 + 0.0605s^4 + 0.0024s^3 + 0.0039s^2 + 0.000079s + 0.000094} \label{eq:magnitude_bandpass_sdom}
\end{align}
...
\begin{lstlisting}
https://github.com/Raghava11020/Signals-and-Systems/blob/main/5/codes/p4.py
\end{lstlisting}
\begin{figure}[htbp]
    \centering
    \includegraphics[width=1\linewidth]{fig/p4.png}
...
\begin{align}
     H_{d,BP}(z) &= \frac{G\brak{1 + 28z^{-2} + 70z^{-4} + 28z^{-6} + z^{-8}}}{1.5799 -10.145z^{-1} + 30.50z^{-2} - 55.412z^{-3} + 66.278z^{-4} - 53.367z^{-5} + 28.291z^{-6} - 9.0629z^{-7} + 1.3594z^{-8}}
\end{align}
where $G=6.39\times 10^{-5}$    
 \begin{lstlisting}
https://github.com/Raghava11020/Signals-and-Systems/blob/main/5/codes/p5.py
\end{lstlisting}
\begin{figure}[htbp]
    \centering
    \includegraphics[width=1\linewidth]{fig/p5.png}
...
\end{align}
\begin{lstlisting}
https://github.com/Raghava11020/Signals-and-Systems/blob/main/5/codes/p6.py
\end{lstlisting}
\begin{figure}[htbp]
    \centering
    \includegraphics[width=1\linewidth]{fig/p6.png}
...
\end{enumerate}
\begin{lstlisting}
https://github.com/Raghava11020/Signals-and-Systems/blob/main/5/codes/p7.py
\end{lstlisting}
\newpage
\begin{figure}[htbp]
    \centering
    \includegraphics[width=1\linewidth]{fig/p7.png}
...
\begin{align}
     \frac{\sin(\omega_{p2} n)}{n\pi} -\frac{\sin\brak{\omega_{p1}n}}{n\pi} &= 2\cos{\brak{\frac{\omega_{p2}n+\omega_{p1}n}{2}}}\sin{\brak{\frac{\omega_{p2}n-\omega_{p1}n}{2}}}\\
            &= \frac{2\cos{\brak{0.39n\pi}}\sin{\brak{0.05n\pi}}}{n\pi}
\end{align}
Multipying by window function we get :
\begin{align}
    h_{BP}\brak{n} = 
\begin{cases} 
   \frac{2\cos{\brak{0.39n\pi}}\sin{\brak{0.05n\pi}}}{n\pi},  & \text{for } -48\leq n \leq 48 \\
    0 &\text{otherwise}
\end{cases} \label{eq:h(n)desired_for_LPF}
\end{align}
\begin{lstlisting}
https://github.com/Raghava11020/Signals-and-Systems/blob/main/5/codes/p8.py
\end{lstlisting}
\begin{figure}[htbp]
    \centering
    \includegraphics[width=1\linewidth]{fig/p8.png}
