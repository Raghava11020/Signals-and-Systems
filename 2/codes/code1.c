...
int main() {
    ...
    float sig1[17];
    int sig2[17]; // Adjusted the size to match sig1
    for (int i = 0; i < 17; i++) {
        sig1[i] = ap_values(5, 2.67 , i);
        sig2[i] = u(i);
    }

    float res[33]; // Changed the size to match the result of convolution
    float sum=0;
    convolve(sig1, sig2, res, 17, 17);
    for (int i = 0; i < 17; i++) { // Adjusted the loop to match res size
        sum += sig1[i];
        fprintf(fp, "%d\t%.2f\t%.2f\n", i, sum, res[i]);
    }
